...
class DummyGPTModel(nn.Module):
    def __init__(self, gpt_config):
        ...
        self.trf_block = nn.Sequential(
            *[DummyTransformerBlock(gpt_config) for _ in range(gpt_config["n_layers"])]
        )
        self.final_norm = DummyLayerNorm(gpt_config["emb_dim"])
        self.out_head = nn.Linear(
            gpt_config["emb_dim"], gpt_config["vocab_size"], bias=False
        )
    ...
    def forward(self, token_batch):
        ...
        token_emb = self.token_emb(
            token_batch
        )  # converts into token embedding (batch_size, context_len, emb_dim)
        pos_embs = self.pos_emb(
            torch.arange(seq_ln, device=token_batch.device)
        )  # gives context_ln x emb_dim
        x = token_emb + pos_embs
        x = self.drop_emb(x)
        x = self.trf_block(
            x
        )  # capturing a list of transformer blocks in sequence block1 --> block2 --> block3
        x = self.final_norm(x)  # scaling / normalizing
        logits = self.out_head(
            x
        )  # dot product of x (batch_size, context_ln, emb_dim) weights (context_ln, emb_dim)
        return logits
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # each token scaled based on the normalized vector hence some token dominate over other
...
class FeedForward(nn.Module):
    ...
    def __init__(self, gpt_config):
        ...
        self.layers = nn.Sequential(
            nn.Linear(
                gpt_config["emb_dim"], 4 * gpt_config["emb_dim"]
            ),  # expanded to 4 time dimension
            GELU(),
            nn.Linear(
                4 * gpt_config["emb_dim"], gpt_config["emb_dim"]
            ),  # shrinking back to original dimension
        )
...
class ExampleDNN(nn.Module):
    ...
    def __init__(self, layer_count, use_shortcut):
        ...
        self.layers = nn.ModuleList(
            [
                nn.Sequential(nn.Linear(layer_count[i], layer_count[i + 1]), GELU())
                for i in range(5)
            ]
        )
...
class GPTModel(nn.Module):
    def __init__(self, gpt_config):
        ...
        self.token_emb = nn.Embedding(
            gpt_config["vocab_size"], gpt_config["emb_dim"]
        )  # index based lookup
        self.pos_emb = nn.Embedding(
            gpt_config["context_length"], gpt_config["emb_dim"]
        )  # create embedding tensort
        self.drop_emb = nn.Dropout(
            gpt_config["drop_rate"]
        )  # randomly assign 0 to some and scale other
    ...
    def forward(self, x):
        batch_sz, seq_len = x.shape  # torch size ( batch_size=1, seq_len=3)
        token_embs = self.token_emb(
            x
        )  # torch size (batch_size=1, seq_len=3, emb_dim=768)
        # print("TOKEN_EMB: ", token_embs.shape)
        pos_embs = self.pos_emb(
            torch.arange(seq_len, device=x.device)
        )  # torch size (seq_len=3, emb_dim=768)

        # print("POS-EMB: ", pos_embs.shape)
        x = token_embs + pos_embs
        x = self.drop_emb(x)  # prevent over reliance on specific token chosen randomly
        x = self.trans_blck(x)
        x = self.final_norm(x)
        # print(
            # "FINAL_NORM shape: ", x.shape
        # )  # torch size (batch_size, seq_len, emb_dim)
        logits = self.out_head(
            x
        )  # torch size (batch_size, seq_len, vocab_size), softmax will give the final probability
...
def generate_model_text(model, x, max_new_tokens, context_ln):
    for _ in range(max_new_tokens):
        ...
        with torch.no_grad():
            ...
            logits = logits[
                :, -1, :
            ]  # get the last token all vocabs ex: (batch_size, vocab_size)
            prob = torch.softmax(logits, dim=-1)
            next_x = torch.argmax(prob, dim=-1, keepdim=True)
            x = torch.cat((x, next_x), dim=1)  # (shape becomes batch, token + 1)
...
if __name__ == "__main__":
    ...
    total_gpt2_params = total_params - sum(
        prm.numel() for prm in model.out_head.parameters()
    )
    ...
    encoded_tensor = torch.tensor(encoded).unsqueeze(0)  # torch.size (1, seq_len)
    print("ENCODED TENSOR SHAPE: ", encoded_tensor.shape)

    # disables dropout
    # disables training behavior
    model.eval()
    output = generate_model_text(
        model=model,
        x=encoded_tensor,
        max_new_tokens=6,
    ...
    text_gen = token_ids_to_text(output, tokenizer)
    print("TEXT GEN USING UTILS: ", text_gen)
