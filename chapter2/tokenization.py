...
def get_data():
    ...
    headers = {
        "User-Agent": "MyApp/1.0 (himi.rockeveryone@gmail.com)"
    }
...
class BaseTokenizer:
    ...
    def encoder(self, txxt):
        ...
        tokens = [self.str_to_int[result[i]] if result[i] in self.str_to_int else self.str_to_int['<|unk|>'] for i in range(len(result)) ]
        return tokens
...
def create_data_loader_v1(txt, batch_size=4, max_length=256, stride=128, shuffle=True, 
...
if __name__=='__main__':
    ...
    dataloader = create_data_loader_v1(raw_txt, batch_size=1, max_length=4, stride=1, shuffle=False)

    dataloader = create_data_loader_v1(raw_txt, batch_size=8, max_length=4, stride=1, shuffle=False)

    data_iter = iter(dataloader)
    inputs, target = next(data_iter) # gives the input tensor and target tensor using sliding window
    print("INPUTS", inputs)
    ...
    context_len = 4
    pos_embedding_layer = torch.nn.Embedding(context_len, output_dim) # create embedding of size output_dim random weights for each batch 
    pos_embeddings = pos_embedding_layer(torch.arange(context_len))  # basicaly maps the vector representation in emebd layer {pos independent}
    ...
    new_dt = create_data_loader_v1(txt1, batch_size=4, max_length=4, stride=1, shuffle=False)
    print("NEW_DT", new_dt)
    iter_new_dt = iter(new_dt)

    for i, it in enumerate(iter_new_dt):
        print(i, it)
    # input_dt, target_dt = next(iter_new_dt)
    # print("INPUT_DT", input_dt)
    # embedding_dt = token_embedding_layer(input_dt)