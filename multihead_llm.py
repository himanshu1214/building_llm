...
from chapter2.tokenization import create_data_loader_v1
vocab_size = 50257
output_dim = 3
txt1 = "Your Journey starts with one step again"
new_data_loader = create_data_loader_v1(txt1, batch_size=1, max_length=6, stride=1, shuffle=False)
...
W_val = torch.nn.Parameter(torch.rand(d_in, d_out), requires_grad=False) # make requires_grad = True when model train for wts updation
...
class SelfAttentionV1(torch.nn.Module):
    ...
    def feed_forward(self, token_embeddding):
        ...
        query = token_embeddding @ self.W_query # step 1 dot  [m x n ] x [n x m ] ~ [m x m ]
...
class CompactCausalAttention(torch.nn.Module):
    def __init__(self, d_in, d_out, dropout, context_ln, qkv_bias=False, ):
        super().__init__()
        self.d_out = d_out
        ...
        self.register_buffer('mask', 
                             torch.triu(torch.ones(context_ln, context_ln), 
                             diagonal = 1)
        )
    ...
    def feed_forward(self, token_embed):
        ...
        cc_attn_wts = torch.softmax(
            cc_attn_scores / cc_keys.shape[1]**0.5, dim = -1
        )
    ...
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=False):
        ...
        self.heads = torch.nn.ModuleList([
            CompactCausalAttention(
                d_in=d_in, d_out=d_out, dropout=dropout, context_ln=context_length, qkv_bias=qkv_bias
            )
        for _ in num_heads]
        )
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, d_in, d_out, context_ln, dropout, num_heads, qkv_bias=False):
        ...
        self.head_dim = d_out // num_heads # ex: 512 Dim, then 512 // 8 ~ 64 Dim each head, total 8 heads
        self.dropout = torch.nn.Dropout(dropout)
        self.d_out = d_out
    ...
    def feed_forward(self, token_embed):
        ...
        mm_attn_wts = torch.softmax(
            mm_attn_scores / mm_keys.shape[-1]**0.5, dim=-1
        )
        ...
        # merge heads ~ (2, 4, 8)
        mm_context_vec = mm_context_vec.contiguous().view(
         b, num_token, self.d_out   
        )
...
mm_context_vec = mha.feed_forward(batch)
