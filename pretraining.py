        ...
        for input_b, target_b in train_data:
            ...
            if global_step % eval_freq == 0:  # optional step
                train_loss, test_loss = evaluate_model(
                    model, train_data, test_data, device, eval_iter
                )
...
def gen_print_sample(model, tokenizer, device, context):
    ...
    with torch.no_grad():
        token_ids = generate_model_text(
            model=model, x=encoded, max_new_tokens=50, context_ln=context_ln
        )
...
def generature_with_decoding_strat(
    model, batch_emb, max_new_tokens, context_ln, temperature, top_k, eos_id=None
):
...
if __name__ == "__main__":
    ...
    log_prob = torch.log(
        torch.cat((target_prob1, target_prob2))
    )  # using torch.log_softmax is more stable
    ...
    test_data_loader = create_data_loader_v1(
        test_data,
        batch_size=2,
        max_length=gpt_config.GPT_PRETRAIN_CONFIG_124M["context_length"],
        stride=gpt_config.GPT_PRETRAIN_CONFIG_124M["context_length"],  # no overlap
    ...
    debug_print(
        "GENRATED TXT BASED ON TEMP SCALING AND TOPK STRAT: ",
